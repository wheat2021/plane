...
- [ ] 1.3 执行 `python manage.py migrate` 并验证字段已创建
...
- [ ] 2.4 验证：使用工号登录成功；使用邮箱登录成功；6位数字但无对应工号时返回错误码 5060
...
- [ ] 3.3 验证：预建账号登录后响应体中 `is_password_reset_required: true`；普通账号为 `false`
...
- [ ] 5.7 验证：导入 5 行含重复数据的 CSV，核对成功/跳过/错误统计正确
...
- [ ] 6.3 验证：选中 3 个用户（1 个已是成员），执行 Action 后确认结果正确
...
- [ ] 8.4 验证：预建账号登录后自动跳转改密页；修改密码后再次登录不再跳转

## 9. 验收测试

- [ ] 9.1 工号登录：用工号 + 初始密码（工号）登录 → 强制跳转改密 → 修改密码 → 正常进入应用
- [ ] 9.2 邮箱登录：现有账号用邮箱 + 密码登录，行为不变
- [ ] 9.3 错误场景：工号不存在时提示用户不存在；密码错误时提示认证失败
- [ ] 9.4 停用账号：Admin 停用账号后，该账号无法登录
- [ ] 9.5 CSV 导入：导入含完整字段的 CSV，验证账号创建、Workspace 分配、密码初始化均正确
