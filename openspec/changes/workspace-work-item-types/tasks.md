...
- [ ] 13.1 验证：以 Workspace Admin 身份访问工作区设置，侧边栏「Work Item Types」入口可见并可点击
- [ ] 13.2 验证：创建新类型（填写名称、选择图标和颜色），类型出现在列表末尾，图标颜色正确显示
- [ ] 13.3 验证：编辑系统类型（如 Bug），可修改图标和颜色，名称字段为只读，保存后全局同步更新
- [ ] 13.4 验证：编辑自定义类型，可修改所有字段
- [ ] 13.5 验证：删除自定义类型前弹窗正确展示影响项目数和工作项数，确认后工作项正确迁移到默认类型
- [ ] 13.6 验证：系统类型（Task/Bug 等）无删除按钮
- [ ] 13.7 验证：拖拽排序后刷新页面顺序持久化
- [ ] 13.8 验证：图标搜索在 query 为空时不渲染列表，输入关键词后正确过滤显示
- [ ] 13.9 验证：非 Workspace Admin 用户无法访问 `/settings/work-item-types` 路由
- [ ] 13.10 验证：项目设置页工作项类型的图标正确从 `logo_props` 读取（不再是硬编码）
- [ ] 13.11 验证：项目设置页系统类型显示 badge 标记
