...
- 升级 `ExtraPropertyDescriptionPopover`：渲染从纯文本改为 Markdown，支持多行、滚动
- 新增 `DefaultPropertyConfigStore`：基于 localStorage 的 MobX store，workspace + issueType 维度存储默认属性的 description
- 在 work item type 展开面板中新增 "Default Properties" 区块，支持 inline 编辑各默认属性的 description
...
- `core/components/issues/extra-properties/description-popover.tsx`（修改）
...
**依赖：** `react-markdown`（已在 apps/web/package.json 中，无需新增）
