# Spec: reference-extra-property-type

## Purpose

定义 `reference` 类型作为 extra property 系统中的一个合法类型，支持存储和展示外部链接列表（`TReferenceItem[]`）。涵盖数据结构、后端校验、前端 Normal 模式编辑器、Compact 模式只读展示、配置界面行为及类型守卫要求。
