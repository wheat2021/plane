...
class ExtraPropertyConfig(BaseModel):
    """
    Stores the configuration for extra properties of an issue type.
    Each IssueType can have multiple ExtraPropertyConfigs, defining additional
    fields that issues of that type should have.
    """
    ...
    key = models.CharField(max_length=100, help_text="Unique identifier for the property within an issue type")
    ...
    def __str__(self):
        return f"{self.issue_type.name} - {self.label}"
