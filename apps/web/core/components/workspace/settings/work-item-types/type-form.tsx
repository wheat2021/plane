...
type FormState = {
  name: string;
  description: string;
  logo_props: TIssueType["logo_props"];
};

const DEFAULT_LOGO_PROPS: TIssueType["logo_props"] = {
  in_use: "icon",
...
export function WorkItemTypeForm({ issueType, onClose }: TTypeFormProps) {
  ...
  const [form, setForm] = useState<FormState>({
    ...
    logo_props: issueType?.logo_props ?? DEFAULT_LOGO_PROPS,
  });
  ...
  return (
    <div className="rounded-lg border border-custom-border-200 bg-custom-background-100 p-4">
      <div className="flex flex-col gap-4">
        ...
        {/* Logo Picker (A2: upstream EmojiPicker) */}
        {!isSystem && (
          <div className="flex items-center gap-3">
            <EmojiPicker
              ...
              onChange={(val) => {
                setForm((prev) => ({ ...prev, logo_props: { ...val } }));
                setIsEmojiPickerOpen(false);
              }}
              label={
                <span className="grid h-9 w-9 place-items-center rounded-md border border-custom-border-200">
                  <Logo logo={form.logo_props} size={16} />
                </span>
              }
            />
            <span className="text-sm text-custom-text-300">
              {t("workspace_settings.settings.work_item_types.form.icon_label")}
            </span>
          </div>
        )}
